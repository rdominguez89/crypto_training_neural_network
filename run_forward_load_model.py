...
while True:
    #load model and scaler, then run forward test
    model_name = input("\nEnter the model filename (without extension): or nothing to end ")
    if len(model_name)<10:break
    if '.pt' in model_name: model_name = model_name.replace('.pt', '')
    ...
    model = torch.load(f'models_{timeframe}/{model_name}.pt', weights_only=False)
    ct = joblib.load(f'models_{timeframe}/ct_{model_name}.pkl')
