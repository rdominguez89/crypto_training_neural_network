...
class ImprovedDualEarlyStopping:
    ...
    def get_best(self):
        return self.best_epoch, self.best_val_acc, self.best_train_acc
...
def run_neural_one(df, side, ratio, remove_columns, timeframe, frac, limit_low_range, limit_top_range, shift_open, use_NY_trading_hour, use_day_month, method, prices_col_1, prices_col_2, period_target, std_dev, multiplier, window, ma_periods, periods_look_back, neurons):
    # Prepare the data
    ...
    for _ in range(1):
        ...
        early_stop = ImprovedDualEarlyStopping(best_val_accuracy, best_train_acc, patience=30, min_epochs=100)
        ...
        for epoch in range(2000):
            ...
            with torch.no_grad():
                ...
                if print_log and ((epoch + 1) % 150 == 0 or epoch == 0):
            ...
            best_epoch, best_val_acc, best_train_acc = early_stop.get_best()
            if best_epoch == 0:
                continue
            if best_val_acc > best_val_accuracy:
                ...
                best_model_state_dict = model.state_dict()  # Save only the state dict
                print(f"New best neurons configuration: {best_neurons} with Val Acc: {best_val_accuracy:.4f}")
                print(f"Final Training Binary Accuracy: {best_train_acc:.4f}")
                print(f"Val Binary Accuracy: {best_val_acc:.4f}, best_epoch: {best_epoch}")
        ...
        if not better_conf_found and best_neurons != 0:
            ...
            print(f"Best neurons configuration: {best_neurons} with Train Acc: {best_train_accuracy:.4f} Val Acc: {best_val_accuracy:.4f} Test Acc: {best_test_acc:.4f}")
            ...
            balance_september, pf_september, usd_win, usd_loss, n_win, n_loss, n_be, n_market, save_model_september = run_forward_test(best_model, ct, timeframe, frac, limit_low_range, limit_top_range, shift_open, use_NY_trading_hour, use_day_month, method, prices_col_1, prices_col_2, period_target, std_dev, multiplier, ratio, side, window, ma_periods, periods_look_back, cols_original, X_train, do_month=8)
            print('\n')     
            break
        elif best_neurons == 0:
